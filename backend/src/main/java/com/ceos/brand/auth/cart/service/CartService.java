...
import com.ceos.brand.auth.order.repository.OrderRepository.UserSnapshot;
...
public class CartService {
    ...
    @Transactional
    public CartResponse checkoutPaidCartForUser(Long userId) {
        User user = userRepository.findById(userId)
            .orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "Usuario no encontrado."));
        return checkout(user);
    }

    private CartResponse checkout(User user) {
        ...
        Order order = orderRepository.create(new UserSnapshot(user.id(), user.name(), user.email()), items);
        cartRepository.clear(user.id());
        marketingEmailService.sendOrderConfirmationEmail(order);
